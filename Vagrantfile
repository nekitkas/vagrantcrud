...
Vagrant.configure("2") do |config|
  ...
  config.vm.define billing_vm do |billing_vm|
    ...
    billing_vm.vm.provision "shell",
      path: "scripts/node-setup.sh"

    billing_vm.vm.provision "shell",
      path: "scripts/run-server.sh",
      env: {
  ...
  config.vm.define gateway_vm do |gateway_vm|
    ...
    gateway_vm.vm.network "forwarded_port", guest: API_GATEWAY_PORT, host: 8080
    ...
    gateway_vm.vm.provision "shell",
      ...
      env: {
        ...
        "INVENTORY_APP_PORT" => INVENTORY_APP_PORT,
        "BILLING_APP_HOST" => billing_vm_addr,
        "BILLING_APP_PORT" => BILLING_APP_PORT,
